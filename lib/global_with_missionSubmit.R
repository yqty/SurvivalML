...
# getSystemCpuUsage, 获取系统目前cpu使用率(仅限linux)
# getSystemMemoryUsage, 获取系统目前内存使用率（仅限linux）
# sendEmail, 批量发邮件
# demoPlotly, 生成测试用PlotlyOutput
# demoplot, 生成测试用PlotOutput
# heatmapDataDemo, 读取heatmap测试数据
# uniqIDWithDate, 生成带有日期的唯一的ID
# uniqID, 生成唯一ID

getSystemCpuUsage <- function(){
  # if linux
  if(any(stringr::str_detect(sessionInfo(),"Linux"))){
    cpu.info <- readLines("/proc/loadavg")
    cpu.usage <- as.numeric(strsplit(cpu.info," ")[[1]][1])
    # print(cpu.usage)
    return(cpu.usage)
  }else{
    # not linux
    return(sample(40:90,1))
  }
}

getSystemMemoryUsage <- function(){
  # if linux
  if(any(stringr::str_detect(sessionInfo(),"Linux"))){
    mem.info <- readLines("/proc/meminfo")
    mem.all <- as.numeric(stringr::str_extract(mem.info[1],"\\d+"))
    mem.available <- as.numeric(stringr::str_extract(mem.info[3],"\\d+"))
    mem.usage.rate <- ceiling(((mem.all - mem.available) / mem.all) *100)
    # print(mem.usage.rate)
    return(mem.usage.rate)
  }else{
    # not linux
    return(sample(30:90,1))
  }
}
...
#### preload data ####
# 读取heatmap测试用数据
# heatmapDataDemo <- read.table("./tmp/expression.txt",header = TRUE, row.names = 1)
...
lzq_GO_merge <- function(GO_pos, GO_neg) {
  ...
  tmp1 <-
    rbind(dd1, dd2) %>% dplyr::select(ID, Description, GeneRatio, type, ONTOLOGY) %>%
...
lzq_GO_plot <- function(tmp, width) {
  tmp_l1 <-
    data.frame(nrow(tmp) - (cumsum(table(tmp$Ontology)) - table(tmp$Ontology) /
                              2))
  tmp_l1$start <- tmp_l1$Freq - table(tmp$Ontology) / 2
  tmp_l1$end <- tmp_l1$Freq + table(tmp$Ontology) / 2
  
  m1 <-
    ifelse(log(max(tmp$Detected), base = 20) * 1000 < 1000, 1000, log(max(tmp$Detected), base = 20) *
             1000)
  m2 <-
    ifelse(log(max(tmp$Enriched), base = 20) * 1000 < 1000, 1000, log(max(tmp$Enriched), base = 20) *
             1000)
  
  p1 <- ggplot(tmp) +
    geom_col(
      mapping = aes(ID, Detected),
      color = "black",
      fill = "#e6b8a2",
      width = 0.75,
      show.legend = F
    ...
    geom_text(
      mapping = aes(ID, Detected, label = Detected),
      hjust = -0.3,
      size = 2.5
    ) +
    scale_y_log10(
  ...
  p2 <- ggplot(tmp) +
    geom_col(
      mapping = aes(ID, Enriched, fill = type),
      color = "black",
      width = 0.75,
      show.legend = F
    ) +
    geom_text(
      mapping = aes(ID, Enriched, label = Enriched),
      hjust = -0.3,
      size = 2.5
    ) +
    scale_y_log10(
  ...
  p0 <- ggplot(tmp) +
    geom_text(
      mapping = aes(ID, 0, label = ID, color = type),
      size = 3,
      show.legend = F,
      hjust = 0
    ) +
  ...
  p3 <- ggplot(tmp) +
    geom_text(
      mapping = aes(ID, 0, label = Description, color = type),
      size = 3,
      show.legend = F,
      hjust = 0
    ) +
...
lzq_KEGG_merge <- function(KEGG_pos, KEGG_neg) {
  ...
  tmp <-
    rbind(dd1, dd2) %>% dplyr::select(ID, Description, GeneRatio, type) %>%
    ...
    dplyr::distinct(Description, .keep_all = T) %>%
    dplyr::mutate(ID = factor(ID, rev(unique(ID))),
                  level3_x = factor(level3, rev(unique(
                    as.character(level3)
                  ))))
  return(tmp)
}
...
lzq_KEGG_plot <- function(tmp, width) {
  tmp_l1 <-
    data.frame(nrow(tmp) - (cumsum(table(tmp$level1)) - table(tmp$level1) /
                              2))
  tmp_l1$start <- tmp_l1$Freq - table(tmp$level1) / 2
  tmp_l1$end <- tmp_l1$Freq + table(tmp$level1) / 2
  tmp_l2 <-
    data.frame(nrow(tmp) - (cumsum(table(tmp$level2)) - table(tmp$level2) /
                              2))
...
lzq_GSEA_merge <- function(GSEA_res) {
  GSEA_res@result <-
    GSEA_res@result[order(GSEA_res@result$NES, decreasing = T), ]
  num1 <- num2 <- 15
  ...
  if (num1 != 0) {
    dd1 <-
      head(GSEA_res@result[GSEA_res@result$NES > 0, ], num1)
  } else{
    dd1 <- GSEA_res@result[GSEA_res@result$NES > 0, ]
  }
  if (num2 != 0) {
    dd2 <-
      tail(GSEA_res@result[GSEA_res@result$NES < 0, ], num2)
  } else{
    dd2 <- GSEA_res@result[GSEA_res@result$NES < 0, ]
  }
...
lzq_refit <- function(testdata) {
  ...
  if (results$method == 'StepCox') {
    rs <-
      lapply(testdata, function(x) {
        data.frame(ID = rownames(x), score = as.numeric(predict(
          results$fit, type = 'risk', newdata = x
        )))
      })
  }
    ...
    rs <-
      lapply(testdata, function(x) {
        data.frame(ID = rownames(x),
                   score = as.numeric(predict(results$fit, newdata = x)$predicted))
      })
  }
  ...
  if (results$method == 'Lasso') {
    rs <-
      lapply(testdata, function(x) {
        data.frame(ID = rownames(x), score = as.numeric(
          predict(
            results$fit,
            type = 'link',
            newx = as.matrix(x),
  ...
  if (results$method == 'SVM') {
    rs <-
      lapply(testdata, function(x) {
        data.frame(ID = rownames(x),
                   score = as.numeric(predict(results$fit, x)$predicted))
      })
  }
    ...
    rs <- lapply(testdata, function(w) {
      ...
      ff <-
        superpc.predict(
          ...
          n.components = results$SuperPC_ncomponents
        )
      rr <- as.numeric(ff$v.pred)
