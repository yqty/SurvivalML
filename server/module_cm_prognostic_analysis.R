...
# module consensus model, analysis prognostic analysis
...
#### user input ####
# cut off value
observeEvent(input$ConsensusModelPageSubmodulePrognosticAnalysisGroupSplitMethodSelection,{
  
  ## show/hide slider input, according to group split method
  if(input$ConsensusModelPageSubmodulePrognosticAnalysisGroupSplitMethodSelection == "custom"){
    shinyjs::show(
...
observeEvent(input$ConsensusModelPageSubmodulePrognosticAnalysisSubmitBtn,{
  
  # return nothing if not clicked
  if(input$ConsensusModelPageSubmodulePrognosticAnalysisSubmitBtn == 0){
    return()
  }
  ...
  # isolate calculation
  input$ConsensusModelPageSubmodulePrognosticAnalysisSubmitBtn
  ...
  isolate({
    ...
    #### set up user input ####
    # plot color palette
    Color_palette <- input$ConsensusModelPageSubmodulePrognosticAnalysisPlotColorSelection
    
    # plot transparency
    alpha <- input$ConsensusModelPageSubmodulePrognosticAnalysisPlotTransparencySelection
    ...
    cutoff <- input$ConsensusModelPageSubmodulePrognosticAnalysisGroupSplitMethodSelection
    percent <- input$ConsensusModelPageSubmodulePrognosticAnalysisCutoffValuePercentSelection
    ...
    ## plot html output
    # cox plot
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultCoxPlotHtml <- renderUI({
      div(
        tags$br(),
        plotOutput(
          outputId = "ConsensusModelPageSubmodulePrognosticAnalysisCoxPlot",
    ...
    output$ConsensusModelPageSubmodulePrognosticAnalysisCoxPlot <- renderPlot({
      return(plot.cox)
    })
    
    # KM plot
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultKMPlotHtml <- renderUI({
      div(
        plotOutput(
          outputId = "ConsensusModelPageSubmodulePrognosticAnalysisKMPlot",
    ...
    output$ConsensusModelPageSubmodulePrognosticAnalysisKMPlot <- renderPlot({
      return(plot.survival.out)
    })
    
    ## download plot
    # download cox
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultCoxPlotDownloadPdfBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
            "BEST_ConsensusModel_SurvivalAnalysis_CoxPlot_",
            self_name,
            "_",
            uniqID(),
            ".pdf",
    ...
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultCoxPlotDownloadPngBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
            "BEST_ConsensusModel_SurvivalAnalysis_CoxPlot_",
    ...
    # create folders
    pdf.dir.name = paste("BEST_ConsensusModel_SurvivalAnalysis_KM_Plot_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    ...
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultKMPlotDownloadPdfBtn <- downloadHandler(
      filename = function(){
    ...
    # generate and save plots
    # create folders
    png.dir.name = paste("BEST_ConsensusModel_SurvivalAnalysis_KM_Plot_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    ...
    output$ConsensusModelPageSubmodulePrognosticAnalysisResultKMPlotDownloadPngBtn <- downloadHandler(
      filename = function(){
    ...
    # # generate plots UI
    # output$ConsensusModelPageSubmodulePrognosticAnalysisResultPlotHtml <- renderUI(
    #   
    #   div(
    ...
    #           width = 4,
    #           plotOutput(
    #             outputId = paste("ConsensusModelPageSubmodulePrognosticAnalysisResultPlot",i,sep="")
    #           )
    #         )
    #       })
    #     )
    ...
    # # generate and save plots
    # # create folders
    # dir.name = paste("BEST_ConsensusModel_PrognosticAnalysis_",geneInput$geneName,"_",uniqID(),sep="")
    # tmp.dir = tempdir()
    ...
    # 
    # lapply(1:nrow(Sur_vars),function(i){
    #   output[[paste("ConsensusModelPageSubmodulePrognosticAnalysisResultPlot", i, sep = "")]] <- renderPlot({
    #     
    #     # save plot
    ...
    # 
    # # download plots
    # output$ConsensusModelPageSubmodulePrognosticAnalysisResultPlotDownloadBtn <- downloadHandler(
    #   filename = function(){
