...
# module prognostic model, analysis immunotherapy
...
#### reactive values ####
prognosticModelModuleImmunotherapyReactiveVals <- reactiveValues(
  score_list = NULL, # recalculated gene score
  Gene_Input = NULL, # user input gene list
  self_name = NULL, # user input gene list name
  select_cohorts = NULL,
...
observeEvent(input$PrognosticModelPageSubmoduleImmunotherapyRecalGeneScoreSubmitBtn,{
  if(input$PrognosticModelPageSubmoduleImmunotherapyRecalGeneScoreSubmitBtn == 0){
    return()
  }
  ...
  input$PrognosticModelPageSubmoduleImmunotherapyRecalGeneScoreSubmitBtn
  ...
  isolate({
    ...
    gtf <- gtf_data$gtf
    results <- PrognosticModelReactiveValues$results
    ...
    # # get user input
    # ######## get gene list ######
    # tmp = input$PrognosticModelPageSubmoduleImmunotherapyGenelistTextArea 
    # 
    ...
    #   prognosticModelModuleImmunotherapyReactiveVals$Gene_Input <- Gene_Input
    # }
    # 
    # ###### get gene list name #####
    # self_name = input$PrognosticModelPageSubmoduleImmunotherapyGenelistNameInput
    # 
    ...
    #   # no error
    #   # save reactive values
    #   prognosticModelModuleImmunotherapyReactiveVals$self_name <- self_name
    # }
    
    # -------------------------------------------------------------------------
    ### 看看多少个队列有这些基因，不包含输入的基因时，则不使用这个队列 ###
    # -------------------------------------------------------------------------
    
    index <- sapply(ICI_data,function(x){mean(Gene_Input%in%rownames(x$expr))==1})
    prognosticModelModuleImmunotherapyReactiveVals$index <- index
    ...
    # save values
    prognosticModelModuleImmunotherapyReactiveVals$select_cohorts <- select_cohorts
    prognosticModelModuleImmunotherapyReactiveVals$select_cohorts2 <- select_cohorts2
    ...
    # save value
    prognosticModelModuleImmunotherapyReactiveVals$score_list = score_list
...
#### gene expression ####
observeEvent(input$PrognosticModelPageSubmoduleImmunotherapyGeneExpTabSubmitBtn,{
  # return nothing if not clicked
  if(input$PrognosticModelPageSubmoduleImmunotherapyGeneExpTabSubmitBtn == 0){
    return()
  }
  
  # return if no valid gene score
  if(any(is.null(prognosticModelModuleImmunotherapyReactiveVals$score_list))){
    sendSweetAlert(
      session = session,
      title = "Error",
  ...
  # isolate calculation
  input$PrognosticModelPageSubmoduleClinlcalAssociationSubmitBtn
  ...
  isolate({
    ...
    Gene_Input <- PrognosticModelReactiveValues$Gene_Input
    gtf <- gtf_data$gtf
    score_list <- prognosticModelModuleImmunotherapyReactiveVals$score_list
    select_cohorts <- prognosticModelModuleImmunotherapyReactiveVals$select_cohorts
    select_cohorts2 <- prognosticModelModuleImmunotherapyReactiveVals$select_cohorts2
    ...
    # get user input
    Color_palette <- input$PrognosticModelPageSubmoduleImmunotherapyGeneExpTabColorSelection
    alpha <- input$PrognosticModelPageSubmoduleImmunotherapyGeneExpTabAlphaSelection
    ...
    # output plot
    output$PrognosticModelPageSubmoduleImmunotherapyGeneExpPlotHtml <- renderUI({
      ...
      div(
        div(
          plotOutput(
            outputId = "PrognosticModelPageSubmoduleImmunotherapyGeneExpPlot",
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyGeneExpPlot <- renderPlot({
      return(plot.geneExp.out)
    })
    
    # download pdf plot
    pdf.dir.name = paste("BEST_PrognosticModel_Immunotherapy_Expression_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyGeneExpPlotDownloadPdfBtn <- downloadHandler(
      filename = function(){
    ...
    # download png plot
    png.dir.name = paste("BEST_PrognosticModel_Immunotherapy_Expression_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyGeneExpPlotDownloadPngBtn <- downloadHandler(
      filename = function(){
...
#### roc ####
observeEvent(input$PrognosticModelPageSubmoduleImmunotherapyRocTabSubmitBtn,{
  # return nothing if not clicked
  if(input$PrognosticModelPageSubmoduleImmunotherapyRocTabSubmitBtn == 0){
    return()
  }
  ...
  # isolate calculation
  input$PrognosticModelPageSubmoduleImmunotherapyRocTabSubmitBtn
  ...
  isolate({
    ...
    # load and import data
    # send reactive values in here
    self_name <- prognosticModelModuleImmunotherapyReactiveVals$self_name
    gtf <- gtf_data$gtf
    score_list <- prognosticModelModuleImmunotherapyReactiveVals$score_list
    select_cohorts <- prognosticModelModuleImmunotherapyReactiveVals$select_cohorts
    select_cohorts2 <- prognosticModelModuleImmunotherapyReactiveVals$select_cohorts2
    index <- prognosticModelModuleImmunotherapyReactiveVals$index
    ...
    # output plot
    output$PrognosticModelPageSubmoduleImmunotherapyRocPlotHtml <- renderUI({
      ...
      div(
        div(
          plotOutput(
            outputId = "PrognosticModelPageSubmoduleImmunotherapyRocPlot",
            #width = paste(div.width,"%",sep=""),
            width = paste0(plot.width,"px"),
            height = paste0(plot.height,"px")
          )
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyRocPlot <- renderPlot({
      return(plot.roc.out)
    })
    
    
    # download pdf plot
    pdf.dir.name = paste("BEST_PrognosticModel_Immunotherapy_ROC_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    pdf.plot.file.path = paste(tmp.dir,"/",pdf.dir.name,sep="")
    dir.create(pdf.plot.file.path,recursive = FALSE)
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyRocPlotDownloadPdfBtn <- downloadHandler(
      filename = function(){
        paste(pdf.dir.name,"*.zip",sep="")
      },
    ...
    # download png plot
    png.dir.name = paste("BEST_PrognosticModel_Immunotherapy_ROC_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    png.plot.file.path = paste(tmp.dir,"/",png.dir.name,sep="")
    dir.create(png.plot.file.path,recursive = FALSE)
    ...
    output$PrognosticModelPageSubmoduleImmunotherapyRocPlotDownloadPngBtn <- downloadHandler(
      filename = function(){
        paste(png.dir.name,"*.zip",sep="")
      },
...
###### user input ######
# cut off value
observeEvent(input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabMethodSelection,{
  
  ## show/hide slider input, according to group split method
  if(input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabMethodSelection == "custom"){
    shinyjs::show(
...
observeEvent(input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabSubmitBtn,{
  # return nothing if not clicked
  if(input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabSubmitBtn == 0){
    return()
  }
  ...
  # isolate calculation
  input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabSubmitBtn
  ...
  isolate({
    ...
    # user input
    # plot color palette
    Color_palette <- input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabColorSelection
    
    # plot transparency
    alpha <- input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabAlphaSelection
    ...
    cutoff <- input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabMethodSelection
    percent <- input$PrognosticModelPageSubmoduleImmunotherapySurvivalTabCutoffValuePercentSelection
    ...
    output$PrognosticModelPageSubmoduleImmunotherapySurvivalTabResultPlotHtml <- renderUI({
      ...
      div(
        plotOutput(
          outputId = "PrognosticModelPageSubmoduleImmunotherapySurvivalTabResultPlot",
          height = plotHeight,
          width = plotWidth
        ),
    ...
    output$PrognosticModelPageSubmoduleImmunotherapySurvivalTabResultPlot <- renderPlot({
      return(plot.survival.out)
    })
    
    # download pdf plot
    pdf.dir.name = paste("BEST_PrognosticModel_Immunotherapy_Survival_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    pdf.plot.file.path = paste(tmp.dir,"/",pdf.dir.name,sep="")
    dir.create(pdf.plot.file.path,recursive = FALSE)
    ...
    output$PrognosticModelPageSubmoduleImmunotherapySurvivalTabResultPlotDownloadPdfBtn <- downloadHandler(
      filename = function(){
        paste(pdf.dir.name,"*.zip",sep="")
      },
    ...
    # download png plot
    png.dir.name = paste("BEST_PrognosticModel_Immunotherapy_Survival_",self_name,"_",uniqID(),sep="")
    tmp.dir = tempdir()
    png.plot.file.path = paste(tmp.dir,"/",png.dir.name,sep="")
    dir.create(png.plot.file.path,recursive = FALSE)
    ...
    output$PrognosticModelPageSubmoduleImmunotherapySurvivalTabResultPlotDownloadPngBtn <- downloadHandler(
      filename = function(){
        paste(png.dir.name,"*.zip",sep="")
      },
