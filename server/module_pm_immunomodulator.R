...
#### start analysis ####
observeEvent(input$PrognosticModelPageSubmoduleImmunomodulatorSubmitBtn,{
  # return nothing if not clicked
  if(input$PrognosticModelPageSubmoduleImmunomodulatorSubmitBtn == 0){
    return()
  }
  ...
  input$PrognosticModelPageSubmoduleImmunomodulatorSubmitBtn
  ...
  isolate({
    ...
    # output$PrognosticModelPageSubmoduleImmunomodulatorHeatmap <-
    #   renderPlotly({
    #     heatmaply::heatmaply(
    #       as.matrix(tmp),
    #       Rowv = FALSE,
    ...
    ##### output #####
    output$PrognosticModelPageSubmoduleImmunomodulatorHeatmapHtml <- renderUI({
      plot.width = paste0((ncol(tmp) * 25 + 300),"px")
      div(
        plotOutput(
          outputId = "PrognosticModelPageSubmoduleImmunomodulatorHeatmap",
          height = "2200px",
          width = plot.width
        ),
    ...
    output$PrognosticModelPageSubmoduleImmunomodulatorHeatmap <- renderPlot({
      return(plot.res.heatmap)
    })
    
    # download file
    output$PrognosticModelPageSubmoduleImmunomodulatorHeatmapDownloadPdfBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
            "BEST_PrognosticModel_Immunomodulator_",
            self_name,
            "_",
            uniqID(),
            ".pdf",
    ...
    output$PrognosticModelPageSubmoduleImmunomodulatorHeatmapDownloadPngBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
    ...
    ##### subplot start #####
    observeEvent(input$PrognosticModelPageSubmoduleImmunomodulatorSubplotSubmitBtn,{
      
      ###### get user input ######
      Cohort = input$PrognosticModelPageSubmoduleImmunomodulatorSubplotCohortSelection
      IMS_ID = input$PrognosticModelPageSubmoduleImmunomodulatorSubplotSignatureSelection
      ...
      IMS_ID2 <- IMS_ID
      cor.method.selected = input$PrognosticModelPageSubmoduleImmunomodulatorSubplotCorMethodSelection
      ...
      # subplot output
      output$PrognosticModelPageSubmoduleImmunomodulatorSubplot <- renderPlot({
        return(plot.cor.plot)
      })
      
      # download subplot
      output$PrognosticModelPageSubmoduleImmunomodulatorSubplotDownloadPdfBtn <- downloadHandler(
        filename = function(){
          str_replace_all(
            paste(
              "BEST_PrognosticModel_Immunomodulator_",
      ...
      output$PrognosticModelPageSubmoduleImmunomodulatorSubplotDownloadPngBtn <- downloadHandler(
        filename = function(){
          str_replace_all(
            paste(
              "BEST_PrognosticModel_Immunomodulator_",
