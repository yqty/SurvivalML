...
observeEvent(input$PrognosticModelPageSubmoduleClinlcalAssociationSubmitBtn,{
  
  # return nothing if not clicked
  if(input$PrognosticModelPageSubmoduleClinlcalAssociationSubmitBtn == 0){
    return()
  }
  ...
  isolate({
    ...
    #### set up user input ####
    # plot color palette
    Color_palette <- input$PrognosticModelPageSubmoduleClinlcalAssociationPlotColorSelection
    
    # plot transparency
    alpha <- input$PrognosticModelPageSubmoduleClinlcalAssociationPlotTransparencySelection
    ...
    ## output plot 
    observeEvent(input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection,{
      
      output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotHtmlOutput <- renderUI({
        
        # # 图像显示宽度
        # div.width = length(names(clin_plot_list[[input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection]])) * 20
        # if(div.width >= 100){
        #   div.width = 100
        # }
        
        plot.num = length(names(clin_plot_list[[input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection]]))
        ...
        div(
          div(
            plotOutput(
              outputId = "PrognosticModelPageSubmoduleClinlcalAssociationResultPlotOutput",
      ...
      output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotOutput <- renderPlot({
        
        # generate plot
        p.out = output_clin_plots[[input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection]]
        
        # set global values
        plot.out$plot <- p.out
        plot.out$plot.num <- length(clin_plot_list[[input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection]])
    ...
    output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotDownloadPdfBtn <- downloadHandler(
      filename = function() {
        fname = paste(
          "BEST_PrognosticModel_ClinicalAssociation_",
          globReactiveValues$homePageCancerSelected,
          "_",
          input$PrognosticModelPageSubmoduleClinlcalAssociationTargetSelection,
          "_",
          uniqID(idLength = 10),
          ".pdf"
        )
    ...
    output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotDownloadPngBtn <- downloadHandler(
      filename = function() {
        fname = paste(
          "BEST_PrognosticModel_ClinicalAssociation_",
          globReactiveValues$homePageCancerSelected,
    ...
    # # generate plots UI
    # output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotHtml <- renderUI(
    #   
    #   div(
    ...
    #           width = 4,
    #           plotOutput(
    #             outputId = paste("PrognosticModelPageSubmoduleClinlcalAssociationResultPlot",i,sep="")
    #           )
    #         )
    #       })
    #     )
    ...
    # # create folders
    # dir.name = paste("BEST_PrognosticModel_ClinicalAssociation_",uniqIDWithDate(),sep="")
    # tmp.dir = tempdir()
    ...
    # # generate plots
    # 
    # # output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlot1 <- renderPlot(
    # #   demoPlot()
    # # )
    # lapply(1:nrow(cancer.dataset()), function(i){
    #   output[[paste("PrognosticModelPageSubmoduleClinlcalAssociationResultPlot",i,sep="")]] <- renderPlot({
    #     #### need mod ####
    #     p = demoPlot()
    #     
    #     # save plot
    ...
    # 
    # # download plots
    # output$PrognosticModelPageSubmoduleClinlcalAssociationResultPlotDownloadBtn <- downloadHandler(
    #   filename = function(){
