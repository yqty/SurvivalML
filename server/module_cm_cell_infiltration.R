...
# module consensus model, analysis cell infiltration
...
#### start analysis ####
observeEvent(input$ConsensusModelPageSubmoduleCellInfiltrationSubmitBtn,{
  # return nothing if not clicked
  if(input$ConsensusModelPageSubmoduleCellInfiltrationSubmitBtn == 0){
    return()
  }
  ...
  input$ConsensusModelPageSubmoduleCellInfiltrationSubmitBtn
  ...
  isolate({
    ...
    ##### output #####
    output$ConsensusModelPageSubmoduleCellInfiltrationHeatmapHtml <- renderUI({
      plot.width = paste0((ncol(tmp) * 25 + 300),"px")
      div(
        plotOutput(
          outputId = "ConsensusModelPageSubmoduleCellInfiltrationHeatmap",
          height = "2200px",
          width = plot.width
        ),
    ...
    output$ConsensusModelPageSubmoduleCellInfiltrationHeatmap <- renderPlot({
      return(plot.res.heatmap)
    })
    
    ##### download file #####
    output$ConsensusModelPageSubmoduleCellInfiltrationHeatmapDownloadPdfBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
    ...
    output$ConsensusModelPageSubmoduleCellInfiltrationHeatmapDownloadPngBtn <- downloadHandler(
      filename = function(){
        str_replace_all(
          paste(
            "BEST_ConsensusModel_CellInfiltration_",
    ...
    ##### subplot start #####
    observeEvent(input$ConsensusModelPageSubmoduleCellInfiltrationSubplotSubmitBtn,{
      
      ###### get user input ######
      Cohort = input$ConsensusModelPageSubmoduleCellInfiltrationSubplotCohortSelection
      Cell_ID = input$ConsensusModelPageSubmoduleCellInfiltrationSubplotCellTypeSelection
      Cell_ID2 <- Cell_ID
      cor.method.selected = input$ConsensusModelPageSubmoduleCellInfiltrationSubplotCorMethodSelection
      ...
      # subplot output
      output$ConsensusModelPageSubmoduleCellInfiltrationSubplot <- renderPlot({
        return(plot.cor.plot)
      })
      
      # download subplot
      output$ConsensusModelPageSubmoduleCellInfiltrationSubplotDownloadPdfBtn <- downloadHandler(
        filename = function(){
          str_replace_all(
            paste(
              "BEST_ConsensusModel_CellInfiltration_",
      ...
      output$ConsensusModelPageSubmoduleCellInfiltrationSubplotDownloadPngBtn <- downloadHandler(
        filename = function(){
          str_replace_all(
            paste(
              "BEST_ConsensusModel_CellInfiltration_",
