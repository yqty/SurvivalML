...
observeEvent(input$AnalysisPagePrognosticModelModuleStartAnalyzeBtn,{
  ...
  isolate({
    ...
    if(length(unique(c(t(tmp))))==1){
      ...
      if(unique(c(t(tmp)))=='Not Significant'){
        cols <- alpha('#F2F2F2',0.9)
      }
    }

    ##输出
    model.score.plot <- Heatmap(
      t(tmp),
      col = cols,
      name = 'Type',
      border = T,
    ...
    output$AnalysisPagePrognosticModelModuleRiskScorePlotHtml <- renderUI({
      plot.width = paste((50 * length(all_cohort_name) + 100),"px",sep="")
      div(
        plotOutput(
          outputId = "AnalysisPagePrognosticModelModuleRiskScorePlot",
          height = paste0((ncol(tmp) * 30) + 50,"px"),
          width = plot.width
        )
      )
    })
    ...
    # plot output
    output$AnalysisPagePrognosticModelModuleRiskScorePlot <- renderPlot({
      return(model.score.plot)
    })
    ...
    # )
    plot.width = nrow(tmp) * 0.4 + 3
    plot.height = ncol(tmp) * 0.5 + 3
    
    # open device
    pdf(paste(pdf.plot.file.path,"/ModelRiskScore.pdf",sep=""), height = plot.height, width = plot.width)
    ...
    #   base_width = 0.3 * length(all_cohort_name) + 1
    # )
    plot.width = nrow(tmp) * 35 + 300
    plot.height = ncol(tmp) * 50 + 200
    
    # open device
    png(paste(png.plot.file.path,"/ModelRiskScore.png",sep=""), height = plot.height, width = plot.width, units = "px")
    ...
    ## table download
    output$AnalysisPagePrognosticModelModuleRiskScorePlotDownloadTableBtn <- downloadHandler(
      
      # filename
      filename = function(){
        
        # output file name
        paste("BEST_PrognosticModel_CandidateGeneList_",uniqID(),".xlsx",sep="")
