...
# -------------------------------------------------------------------------
##可自定义选择数量 50-1000
select_pcor_ids <- cordata$ENTREZID[2:1001]
select_ncor_ids <- cordata$ENTREZID[nrow(cordata):(nrow(cordata)-999)]
...
Pathway_ID <- 'Arachidonic acid metabolism'

source('lzq_gseaplot.R')
options(digits = 2)
lzq_gseaplot(GSEA_KEGG,Pathway_ID,'firebrick')
ggsave(filename = 'gsea8.pdf',width = 4,height = 4)
